...
- **Main Features**:
  ...
  - **道場**：学習センター内の演習型トレーニング機能。全16演習の自動採点・添削、稽古履歴ライブラリ、自作演習、段位システム、成長ダッシュボード、**道場全体を横断検索・閲覧できる稽古ライブラリページ（NEW）**
  - 執筆サポート：学習センター／ツール／テンプレート／書斎／企画ラボ／**メモ（NEW）**
...
- **Local Dev**:
  ...
  ```
- **Last Updated**: 2026-08-19（道場「稽古ライブラリ」新設：練習結果・提出文章の横断集積ページを追加）

## モバイル版 大幅構造改革 第4弾：ホーム(top)ページの独自レイアウト（NEW・完了）
第3弾の「今後の予定」に挙げていた、ホームページのモバイル専用レイアウト再設計に着手した。従来はPC用の2カラムグリッド（`.top-main-grid`）をそのまま縦積みにしただけで、各ウィジェット内部の構造もPC版と同一のままだった。「Webサイトを縮小しただけ」の象徴的な箇所だったため、CSSのみでモバイル専用の情報設計に組み替えた（既存のHTML構造・JS・PC版表示は一切変更していない）。

### 実装内容（4項目、いずれも`@media (max-width:900px)`内に閉じたCSS変更のみ）
- **① クイックアクションの横スクロールピルカルーセル化**：`.top-quick-actions`（執筆記録/新規作品/タスク追加/インスピレーションの4ボタン）を`flex-wrap:nowrap; overflow-x:auto`の横スクロール化。スクロールバーは`scrollbar-width:none`／`::-webkit-scrollbar{display:none}`で視覚的に隠しつつ、タッチスクロール操作性は維持（既存の横スクロールカルーセルパターンを踏襲）
- **② 今日の執筆進捗のヒーロー的強調**：`.top-wp-count`（文字数の数字）を34pxに拡大し、進捗バーの高さも10pxに太くして、ホーム画面で最初に目に飛び込む「今日どれだけ書いたか」を強調
- **③ 7日間スケジュールの横スクロール日別カードストリップ化**：`.top-week-schedule`を縦積みの行リストから、日付ごとに独立した幅108pxのカード（`.top-week-row`）が横に並ぶストリップ表示へ変更。各カードは`flex-direction:column`で日付ラベルの下にタスクチップが縦に並ぶ構成にし、カレンダーアプリの週表示に近い見た目にした
- **④ プロジェクト行・タスク行のタップ領域・カード感強化**：`.top-project-row`／`.top-task-item`に`min-height:52px`の最低タップ領域を確保し、軽い影（`box-shadow`）でカードらしさを付与。`:active`時に`scale(0.98)`で押下フィードバックを追加し、タップ可能であることを視覚的に伝える

### 検証
- CSS波括弧対応チェック（open=close=2764）
- ビルド成功（`dist/_worker.js` 39.85kB、`app.css` 391102→268535 bytes/31%削減）、PM2再起動・HTTP 200確認
- 一時的なE2E診断コード（`_e2etest=homelayout`）を`init()`末尾に追加し、実際に`navigate('top')`を実行した上で対象要素の`getComputedStyle`をPlaywrightで検証。以下全項目PASSを確認後、診断コードは完全削除（diffクリーン化を確認済み）：
  - `.top-quick-actions`：`overflow-x:auto`、`display:flex`、`flex-wrap:nowrap`
  - `.top-week-schedule`：`flex-direction:row`、`overflow-x:auto`
  - `.top-week-row`：`width:108px`、`flex-direction:column`
  - `.top-wp-count`：`font-size:34px`
  - `.top-project-row`：`min-height:52px`
- Playwrightコンソールキャプチャ（公開URL経由）でエラー0件を最終確認

## モバイル版 大幅構造改革 第3弾：スタックナビゲーション化（完了）
「今のモバイル版はサイトを無理やりモバイルアプリにしたような感じ」という指摘を受け、モバイル版のUI/UXを飛躍的・抜本的に改良するシリーズの第3弾。既存のモバイル対応（ボトムシートモーダル・エッジスワイプ戻る・タッチフィードバック統一等）を土台にしつつ、「Webサイトっぽさ」の核心的な原因だった以下2点に着手した。

### ① ページ遷移のスタックナビゲーション風スライド化（完了）
- 従来はどのページ遷移も同じ弱いフェード＋わずかな縦移動（`mobilePageIn`）のみで、「進む」「戻る」の区別がなく、ネイティブアプリのような方向性のある画面遷移になっていなかった
- `navigate()`に`_navDirection`（`'forward'` / `'back'`）を追加。`popstate`経由（ブラウザ/Androidの「戻る」操作）で呼ばれた場合は`'back'`、それ以外の通常遷移は`'forward'`として記録
- `#page-content`に`data-nav-dir`属性として出力し、CSS側で「進む」時は右から、「戻る」時は左からスライドインするアニメーション（`mobilePageSlideInFromRight` / `mobilePageSlideInFromLeft`）を適用
- `prefers-reduced-motion: reduce`環境では引き続きアニメーション無効化を維持

### ② モバイル専用「戻るボタン中心」階層型ヘッダー（完了）
- 従来はどの階層のページを開いても、PC版と同じ「ハンバーガー＋ロゴ＋アイコン付きページタイトル＋副題」という情報量の多いトップバー構成のままで、「今どこの下にいるか」「どうやって前の画面に戻るか」が直感的でなかった
- ボトムナビの5大タブ（ホーム／創作ホーム／タスク／メモ／学ぶホーム）は階層の「根」として従来どおりロゴ表示を維持
- それ以外の階層ページ（学習センター・ツール・テンプレート・書斎・企画ラボ（タネ含む）・名前辞典・世界観設計・設定等）では、ハンバーガー・ロゴ・アイコン付きタイトルを非表示にし、代わりに「戻る矢印ボタン＋現在ページ名のみ」というシンプルな階層ヘッダー（`.topbar-mobile-back`）に切り替え
- 戻るボタン（`mobileGoBack()`）は`history.back()`を呼ぶだけで、実際の画面遷移・スライド方向の切り替えは既存の`popstate`ハンドラが担当するため、通常のナビゲーションと完全に一貫した動作になる
- 作品ワークスペース（プロジェクトを開いている状態）は保存ボタン等の専用UIが別にあるため、今回のスコープ外（次回以降で検討）

### 今後の予定（次回セッション以降）
- 作品ワークスペース（proj配下のページ）への戻るボタン中心ヘッダーの適用検討
- タップ/プレスフィードバックとマイクロインタラクションの全体的な統一の見直し
...
### リッチエディタツールバーのモバイル対応（追加改良・完了）
ツールバーのボタン数が増加(見出し・小見出し・太字・斜体・下線・取り消し線・箇条書き・番号付きリスト・引用・リンク・元に戻す・やり直し・書式クリアの13種程度)したことに伴い、狭い画面幅でも操作しやすいよう`app.css`に600px以下用のレスポンシブ調整を追加しました。
- `.hub-rich-toolbar`：ボタン間の余白(gap)とパディングを縮小し、折り返し時の見た目を改善
- `.hub-rich-btn`：タップしやすいよう幅・高さをわずかに拡大(28x26px → 32x30px)しつつフォントサイズを微調整
- `.hub-rich-editor`：モバイルでの縦スペースを考慮し、最小/最大高さとパディングを縮小
- `.hub-rich-footer`（文字数カウンター行）：パディングを縮小
- 既存の他要素のレイアウトには影響しないよう、リッチエディタ関連クラスのみをスコープしたメディアクエリとして追加

### 対応状況（モバイル対応分）
- CSSブレース対応チェック・JS構文チェック（`node -c`）・ビルド（`vite build`）・PM2再起動・HTTP稼働確認（200 OK）・実ブラウザでのコンソールエラー確認（Playwright、エラー0件）まで完了済み

## テンプレート集：全31種を専用UI化（完了）
...
## 道場「稽古ライブラリ」— 練習結果・提出文章の横断集積ページ（新規追加）
ユーザー要望「学習センターの『道場』での練習結果・文章/内容をちゃんと集積できるようにしてほしい」に基づき、フェーズ4で実装済みの`DojoLibraryDB`（演習ごとの提出履歴）はあったものの、**道場全体を横断して全ての提出文章・添削結果を検索・閲覧できる集積ページが欠けていた**ギャップを埋めるため、新規ページ「稽古ライブラリ」（`dojo-library`）を追加しました。

### 実装内容
- **`renderDojoLibraryPage()`**：`DojoLibraryDB.allEntries()`で全演習・全カテゴリ横断の提出ログを取得し、カード形式で一覧表示。各カードに得点・カテゴリ・難易度・提出日時・解答冒頭プレビュー・文字数・自己ベスト王冠を表示
- **検索・絞り込み・並び替え**：キーワード検索（演習名／解答本文／添削コメント全文対象）、カテゴリ別・難易度別フィルタ、新しい順／古い順／高得点順／低得点順のソート。フィルタ条件は`DB`（永続化レイヤー）経由で保存され、ページ再訪時も維持
- **`openDojoLibEntry()`**：カードクリックで提出した解答全文と添削結果（採点基準別評価・総合コメント）をモーダルで全文表示。モーダルから該当演習ページへ直接遷移、またはその場で提出記録を削除可能
- **ヘッダー統計**：総稽古数・全体平均点・累計執筆文字数をページ上部に常設表示
- **空状態ハンドリング**：まだ提出履歴が無い場合は「道場へ行く」導線付きの案内を表示
- **導線の追加（3箇所）**：①成長ダッシュボードに「稽古ライブラリを見る」リンク、②道場トップバナーに「稽古ライブラリ」ボタン、③個別演習ページの履歴パネルに「全体ライブラリへ」ボタン
- ルーティング（`render()`／`renderLayout()`のモバイル戻るヘッダー判定／`TOPBAR_PAGES`／`renderLearnPage()`）に完全統合し、既存ページと同じ操作感で遷移可能

### 対応状況
- JS構文チェック（`node -c`）・CSS括弧バランスチェック・ビルド（`vite build`）・PM2再起動・稼働確認（200 OK）まで完了済み
- Playwrightによる実ブラウザE2E検証を実施し、全項目PASSを確認：テストデータ投入→ライブラリページ表示（カード数）、キーワード検索での絞り込み、カテゴリ絞り込み、高得点順ソート（正しく最高得点が先頭）、フィルタ解除後の全件表示、モーダルでの全文表示（解答本文が正しく含まれる）、個別演習ページ・道場トップからの導線ボタンの存在、を全て確認
- 検証用の一時診断コードはE2E確認後に完全削除し、diffには機能実装部分のみが残ることを確認済み
- 稽古ライブラリ専用CSS（`.dojo-lib-*`、`.dojo-btn-library`）を新設・反映済み

## 世界観設計ノート：プロジェクトごとのデータ分離＋大幅機能拡充（完了）
「世界観設計」を、複数の作品（プロジェクト）を横断する単一のグローバルデータから、**作品ごとに独立した設定データ**へ抜本的に再設計しました。あわせてリッチテキスト化・自動保存・「場所」タブの新設・資料エクスポートを追加しています。

### プロジェクトごとのデータ分離（重大な設計欠陥の修正）
- 従来は`DB.get('worldbuilding', ...)`というグローバル単一キーに全プロジェクト共通で保存されており、複数作品を管理していても世界観設定が1つしか持てない欠陥があった
- `proj.worldbuilding`フィールドへ移行し、作品ごとに完全に独立した世界観データを持てるように修正（`getProjWorldbuilding(proj)`）
- 既存のグローバルデータは、初回アクセス時に「最初のプロジェクト」へ一度だけ自動移行（`wb_migrated`フラグで判定、移行後は即座に永続化）。他の作品には影響せず、データが重複コピーされることもない

### リッチテキスト化＋自動保存の統一適用
- 基本設定タブ（世界観タイトル・時代・舞台・世界のルール・地理・文化・政治・科学技術・歴史・葛藤の10項目）と社会・政治タブ（社会構造・経済・法律・宗教・教育・軍事の6項目）を、他9つのメモ・ステッカー系ハブと同じリッチテキストエディタ（`renderHubRichEditor`）に統一
- 入力すると600ms後に自動保存（`hubAutoSave`）。現在表示中のタブのフィールドのみを一括読み取りして保存する設計（`persistWbMainFields`）

### 新規タブ「場所・ロケーション」の追加
- 場所名・種類（都市/建物/自然地形/国・地域/異空間/その他）・説明・雰囲気・関連する勢力や住人をカード形式で管理
- カードクリックで編集モーダルを開き、内容を更新・削除できる（`openAddWbLocation` / `openEditWbLocation` / `saveWbLocationEdit` / `deleteWbLocation`）

### 用語集・勢力・年表のプロジェクトスコープ化
- 用語集（`glossary`）・勢力・組織（`factions`）・歴史・年表（`timeline`）も、旧グローバルキー方式から`proj.worldbuilding`配下へ移行し、作品ごとに独立管理できるように再実装
- 年表は年代順に自動ソートしてタイムライン表示

### 世界観データ全体のMarkdown資料エクスポート（新規）
- ヘッダーの「資料として書き出す」ボタンから、基本設定・社会設定・場所・勢力・年表・用語集を全てMarkdown形式でまとめてダウンロード可能（`exportWorldBuilding`）
- 執筆時の参照資料や、共同制作者への共有資料としてそのまま使える構成

### 用語集・勢力・年表・場所の相互リンク機能（新規・完了）
- 用語集・勢力・場所・年表の説明文（`def`/`desc`/`goal`/`impact`）の中に、他の登録済みエントリの名前（用語・勢力名・場所名・出来事名）が含まれている場合、自動でクリック可能なリンクに変換（`linkifyWbText` / `getWbLinkTargets`）
- リンクをクリックすると該当タブへ自動的に切り替わり、対象のカード/行までスムーススクロールしてハイライト表示（`jumpToWbEntry`）。カード全体がクリック領域を持つ場所カードでも、リンククリック時は`stopPropagation`で編集モーダルの誤起動を防止
- 名前が長い順にマッチさせることで、短い名前が長い名前の一部に誤反応する問題を回避
- 実装のついでに、年表タブ（`renderWbTimelineTab`）で表示用に年代順ソートした配列のインデックスをそのまま削除・リンク先IDに使っていた既存の潜在バグ（並び替え後のindexと元配列のindexがズレて、意図しない項目を削除/リンクしてしまう可能性）を修正し、元配列のindexを保持する方式に変更
- 相互リンクの見た目用CSS（`.wb-xlink`、ハイライト時の`.wb-entry-highlight`アニメーション）を`app.css`に追加

### 対応状況
- JS構文チェック・ビルド（`vite build`）・PM2再起動・稼働確認（200 OK）まで完了済み
- 実ブラウザでのスクリーンショット目視確認・クリック操作確認は未実施（サンドボックスのPlaywright用Chromiumバイナリのダウンロードがネットワーク不安定によりタイムアウト）。コードレビュー（関数シグネチャ・呼び出し元整合性・既存の確立パターンとの一致）で品質を確認済み

## テンプレート一覧の種別バッジ表記を削除（完了）
テンプレート一覧カードおよびヒントテキストに表示していた「フォーム対応」「チェック式」「読む」の種別バッジ表記を削除しました（`renderTemplateCardHtml`）。テンプレートの分類自体（フォーム型／チェックリスト型／コンテンツカード型）や各UIの機能はそのまま維持しています。

### 対応状況
- JS構文チェック・ビルド（`vite build`）・PM2再起動・稼働確認（200 OK）まで完了済み

## 設定ページ拡充③④：削除確認の強化と執筆目標の履歴・達成率トラッキング（完了）

### ③ 全データ削除の確認強化
- `confirmClearAllData()` を拡張し、削除前に削除対象の件数（プロジェクト数・稿数・登場人物数・執筆日誌件数）を明示表示
- 「削除」というキーワードをテキスト入力欄に一致するまで入力しないと「すべて削除する」ボタンが押せないタイプ確認方式を追加（`oninput`でボタンの`disabled`属性を切り替え）
- `clearAllData()` 側にも `disabled` チェックのガードを追加し、二重の安全策とした
- `.btn:disabled` のスタイル（`opacity:0.4; cursor:not-allowed; pointer-events:none;`）を `app.css` に新規追加

### ④ 執筆目標の履歴・達成率トラッキング強化
- 新規のlocalStorageキーは追加せず、既存の `journal_entries`（執筆日誌）データから直近30日分の執筆量を動的に集計する設計を採用（データの二重管理を避けるため）
- 設定ページの「執筆目標」カードに以下を追加：
  - 直近30日の目標達成率（%）、目標達成日数（達成日数/記録日数）、連続執筆日数（既存の`calcWritingStreak`関数を再利用）の3指標を表示
  - 直近30日間の日次執筆量をミニバーチャートで可視化。目標達成日は緑（`var(--matcha)`）、未達成日は薄紫（`var(--fuji-border)`）、記録なしの日は薄灰（`var(--bg-hover)`）で色分け
  - 執筆日誌の記録が一つも無い場合は、執筆日誌ページへの誘導リンクを表示

### 対応状況
- JS構文チェック・ビルド（`vite build`）・minify（`scripts/minify-static.mjs`）・PM2再起動・稼働確認（200 OK）まで完了済み
- 実ブラウザでのスクリーンショット目視確認・クリック操作確認は未実施（サンドボックスのPlaywright用Chromiumバイナリのダウンロードがネットワーク不安定によりタイムアウト）。コードレビュー（関数シグネチャ・呼び出し元整合性・既存の確立パターンとの一致）で品質を確認済み

## キャラクター機能の大幅強化（完了）
ダッシュボードの「キャラクター」機能を、既存のシンプルなカード+テキストリスト形式から、9ハブと統一感のあるリッチテキスト対応・変容トラッキング・ビジュアル関係図を備えた本格的な設計ツールへ強化しました。

### リッチテキストエディタへの統一
- 外見・風貌／バックストーリー・過去／口癖・話し方の3フィールドを、他9つのメモ・ステッカー系ハブと同じリッチテキストエディタ（`renderHubRichEditor`）に変更
- 入力すると600ms後に自動保存（`hubAutoSave`）。プレーンテキスト版（`ch.back`等）もHTML保存と同時に同期し、検索・エクスポート・カード上の完成度計算からは変わらず利用可能

### キャラクターアーク・トラッキング（新設）
- Before（幕開けの姿）→ 転機（変化のきっかけ）→ After（変容後の姿）の3ステージで、キャラクターの変容の弧を設計・記録できる機能を追加（`getCharArcStages` / `saveCharArcStage`）
- 一覧カードに「アーク○/3」の進捗バッジを表示し、3ステージすべて記入すると視覚的に強調表示

### 人物関係図のビジュアル化（テキストリスト → SVGグラフ）
- 従来のテキストリストのみだった人物関係図を、キャラクターを円状に配置し関係タイプ別の色・線種で結ぶSVGビジュアルグラフに強化（`renderRelationshipMap`）
- 関係の種類（恋愛・友人・ライバル・親子・兄弟・師弟・上司部下・敵・その他）ごとに配色・線種を定義した凡例を表示。ノードクリックでそのキャラクターの詳細ページへ遷移
- 従来通りの編集用リスト（削除ボタン付き）もグラフの下に維持し、既存の操作性を損なわない

### キャラクター詳細ページ化（モーダル統合）
- 従来の単純な編集モーダル（`openEditCharModal`）を廃止し、タブ切替式の「キャラクター詳細ページ」（基本情報／内面・アーク／関係性）に統合（`openCharDetail`）
- 「関係性」タブでは、そのキャラクターが関わる関係を一覧表示し、直接追加・削除・相手キャラクターの詳細へジャンプが可能
- 削除操作をシングルクリックの即時削除から、確認モーダルを介する方式に変更（関連する人物関係も同時に削除されることを明示）

### 潜在バグの修正
- 追加/編集モーダルの役割選択肢の表記「敵・antagonist」が、ロール別色設定`CHAR_ROLE_COLORS`のキー「antagonist（敵）」と不一致で、この役割を選択してもカードの色・アイコンが適用されない不具合があった。単一の定数`CHAR_ROLE_OPTIONS`から選択肢とカラーマップの両方を生成する方式に統一して解消
- 同じ表記不一致が、初回起動時に投入されるサンプルプロジェクトの初期データ（キャラクター「田中 教授」の役割）にも存在していたため、合わせて修正

### 登場人物リストのエクスポート内容を拡充
- `exportCharacterList`の出力に、外見・バックストーリー・口癖・キャラクターアーク・関係性の情報を追加

### 対応状況
- JS構文チェック・ビルド（`vite build`）・minify（`scripts/minify-static.mjs`）・PM2再起動・稼働確認（200 OK）・PlaywrightConsoleCaptureでのページロード確認（コンソールエラー無し）まで完了済み
- 実ブラウザでの実際のクリック操作によるE2Eテスト（詳細ページのタブ切替、アーク入力、関係図のノードクリック等）は下記の理由により未実施。コードレビュー（データ構造・関数呼び出し元の整合性、既存の確立パターンとの一致）で品質を確認済み

## キャラクター機能：深み診断・キャスト分析・並び替え・絵文字ピッカーの追加（完了）
上記の大幅強化に続き、既存ツール「キャラクター診断シート」（`runCharDiag`）の診断ロジックをダッシュボードのキャラクター機能本体に統合し、さらに使い勝手を向上する改良を追加しました。

### キャラクター深み診断の統合（新設タブ）
- Want/Need/ウーンド（傷）/ライ（誤信念）/トゥルース（真実）/ボイス（口癖）/禁止語彙/ゴースト（自己破壊的パターン）の8要素から立体度スコア（0〜100点）・推定アークタイプ（フラットアーク／ポジティブアーク）・Want×Need対立強度を算出する共通関数`computeCharDepth`を新設し、既存の単発ツールと同じロジックを`proj.characters[]`のデータから直接計算できるようにした
- キャラクター詳細ページに「深み診断」タブを新設（`renderCharDiagPanel`）。ウーンド・ライ・トゥルース・禁止語彙・ゴーストの5項目を新規入力欄として追加し、入力するたびにスコア・アークタイプ・改善提案がその場で再計算される（入力欄自体は再描画せず、スコア表示部分のみ差し替えることでフォーカスを維持）
- キャラクターカードの完成度バー・アバター右下バッジ、詳細ページのタイトルにも深み診断スコアを表示

### キャスト全体分析（一覧ページ）
- キャラクター一覧の上部に、キャスト全体の平均深み診断スコアと、以下の警告を表示するサマリーパネルを追加（`computeCastInsights`）
  - 「主人公」役割のキャラクターがいない／複数いる場合の指摘
  - 2人以上のキャラクター間でWantまたはNeedの文言が完全一致している場合の重複警告
  - 深み診断スコアが低い（30点未満）キャラクターの指摘

### カードの並び替え機能
- 「全員・グリッド表示」時のみ、各カードに上下移動ボタンを表示し、`proj.characters[]`内の並び順をドラッグ無しで入れ替えられるように（`moveCharacter`）

### 絵文字クイックピッカー
- 詳細ページの絵文字入力欄に「よく使う絵文字から選ぶ」ボタンを追加し、人物アイコンによく使われる絵文字30種をワンクリックで設定できるように（`toggleCharEmojiPicker` / `setCharEmoji`）

### エクスポート・サンプルデータへの反映
- `exportCharacterList`の出力に深み診断スコア・推定アークタイプ・ウーンド/ライ/トゥルース/禁止語彙/ゴーストの各項目を追加
- 初回起動時のサンプルプロジェクト（「木村 拓也」「田中 教授」）にもウーンド/ライ/トゥルース/禁止語彙/ゴーストのデモ値を追加し、新機能を初見で確認できるように

### 対応状況
- JS構文チェック・ビルド・minify・PM2再起動・稼働確認（200 OK）・PlaywrightConsoleCaptureでのページロード確認（コンソールエラー無し）まで完了済み
- 実ブラウザでの実際のクリック操作によるE2Eテスト（深み診断タブの入力・並び替えボタン・絵文字ピッカー）は、下記の「未完了・引き継ぎ」に記載の理由により本セッションでは未実施。コードレビューで整合性を確認済み

## 企画ラボ（完了） — 着想から企画までのストリーム型ダッシュボード
「執筆サポート」セクション（モバイル版では「書斎」枠内）に新設した、作品化される前の"アイデアの種"を蒔き・育て・企画書として実らせるための専用ダッシュボード。既存の「作品ダッシュボード」（`proj`構造・12フェーズ固定のワークフロー型）とは意図的に設計思想を分け、時系列に流れ続ける**ストリーム型**のデータモデル・UXを採用した。CINÉ FORGE（別ツール）のソースコードを参考例として検討したが、コピーはせず、シナリオラボの世界観（明朝体、CSS変数によるカラーパレット、カードUI）に合わせて独自設計した。AI(LLM)呼び出しは行わず、既存の`generateLoglines`/`generatePitch`と同系統のロジックベース（ヒューリスティック・テンプレート補間方式）で全機能を実装している。

### データモデル：「タネ（Seed）」とストリーム
- 保存先は独立ストア`planlab_seeds`（`PlanLabDB.getSeeds/saveSeeds/getSeed/saveSeed/deleteSeed`）。作品プロジェクト(`proj`)とは完全に分離した軽量データ構造
- 1件の着想＝「タネ」（`newPlanSeed`）。`stage`は種→発芽→つぼみ→開花→企画書の5段階（`PLANLAB_STAGES`）を遷移し、固定フェーズを順番にこなす作品ダッシュボードとは異なり、ユーザーが自由に行き来できる
- 各タネは時系列の`growthLog[]`（成長ログ）を蓄積するストリーム構造を持つ。ログ種別（`PLANLAB_LOG_KINDS`）は「着想の種まき」「メモ」「視点変換の記録」「ログライン選定」「企画書生成」「ステージ変更」の6種で、タネの成長過程がそのままタイムラインとして可視化される

### UI構成
- **ヒーローバナー＋クイック着想投入欄**：思いついた瞬間に一文を打ち込むだけで新規タネが生まれる（`planlabQuickSow`）
- **ステージフィルターチップ**：5ステージ＋「すべて」でストリームを絞り込み表示（`planlabSetFilter`）
- **成長ストリーム**：タネカードの一覧。各カードにステージバッジ・最新ログの抜粋・進行度バーを表示（`renderPlanLabSeedCard`）
- **タネ専用ダッシュボードページ（NEW）**：モーダルポップアップ(`openPlanSeedHub`)を廃止し、URLナビゲーション可能な独立ページ`planlab-seed`に刷新（`navigateToPlanSeed(id, activeKey)`で遷移、`State.currentSeedId`で表示中のタネを管理、`history.pushState`でブラウザの戻る/進むにも対応）。`renderPlanLabSeedDash(s)`が作品ダッシュボード（`renderProjectDash`）と同じ`pdv3-*`のヒーロー＋統計＋2カラムボディ構造を踏襲しつつ、企画ラボ独自の「ステージロードマップ」（`pls-road-*`、クリックで各ステージへ直接ジャンプ）・「育成度サマリー」（6軸バッジ、クリックで該当タブへ）・「成長の軌跡」タイムラインプレビュー・登場人物プレビューを追加し、作品ダッシュボードを上回る情報密度を実現。`renderHubTabs('planlab-seed-hub', ...)`による**8タブ構成**（`renderHubTabs`、下記参照）：
  1. **育てる**：タイトル・ステージ・タグの編集、自由メモの追記、成長ログのタイムライン表示に加え、**育成度スコアカード**（下記「育成度診断」参照）と「次に育てるとよさそうなこと」提案リストを表示。提案クリックで該当タブへ直接ジャンプできる
  2. **コンセプト（NEW）**：テーマ・プレミス・トーン・ターゲット・USPの5項目を編集（`renderPlanLabConceptPanel`）。作品ダッシュボードの`renderConcept`と同一の選択肢セット（`PLANLAB_TONE_OPTIONS`/`PLANLAB_TARGET_OPTIONS`）を採用し、記入をワンクリックで成長ログに記録できる（`planlabSaveConceptToLog`）
  3. **視点を変える（強化）**：AIを使わない固定発想フレーム6種（主客転換／時間を広げる／制約を外す／究極まで縮める／対極をぶつける／当事者の声）をワンクリック適用（`planlabApplyLens`。適用するとステージが「種」なら自動的に「発芽」へ昇格）。**試した視点数バッジ**（例：`3/6`）を表示し、試用済みレンズには✓マークとハイライト（`.planlab-lens-card.tried`）を付与、全視点を試すと祝福バナーが出る
  4. **ログライン工房（強化）**：作品ダッシュボードの`generateLoglines()`と同等の設計に刷新。主人公・欠点(flaw)・ゴール・障害・テーマの5項目から**7パターン**のログライン候補をテンプレート補間生成し、各候補に文字数カウントと品質判定（65字以内=◎最適／80字以内=△やや長め／それ以上=✕長すぎ）を表示。個別コピー・全コピーボタン、最適パターンのハイライト表示に対応（`planlabGenerateLoglines`/`planlabCopyLogline`/`planlabCopyAllLoglines`）
  5. **キャラクター（NEW）**：企画段階の人物を名前・役割・一言紹介・Want・Needで登録・編集・削除できる（`renderPlanLabCharacterPanel`/`planlabOpenAddCharacter`/`planlabSaveCharacter`/`planlabDeleteCharacter`）。各人物カードにWant/Need等の充足率から算出した簡易深みスコアを表示（`planlabComputeCharSimpleDepth`）
  6. **あらすじ（NEW）**：`renderHubRichEditor`（見出し／太字／箇条書き等のツールバー付きcontenteditable）を使ったリッチテキストであらすじを執筆できる（`renderPlanLabSynopsisPanel`/`planlabSaveSynopsis`）。マイルストーンとして成長ログに記録すると「つぼみ」以降へ自動昇格（`planlabLogSynopsisMilestone`）
  7. **企画書（強化）**：作品ダッシュボードの`generatePitch()`と同等の構成に刷新。ジャンル・フォーマット・主人公（キャラクタータブの先頭人物から自動プリフィル）・テーマ／ターゲット（コンセプトタブから自動プリフィル）・作者コメントを加味し、あらすじ・登場人物一覧・成長軌跡を含む本格的な企画書ドキュメントを生成（`planlabGenerateBrief`）。生成すると「企画書」ステージへ昇格
  8. **送る**：作品タイトルを指定して「作品ダッシュボード」へ新規プロジェクトとして送出するブリッジ機能（`planlabSendToProject`。`newProject`でタイトル・ジャンル・ログライン・あらすじに加えて**コンセプト（`proj.concept`）とキャラクター一覧（`proj.characters`、`addCharacter`と同一のフルデータ構造に変換）も引き継いで**新規作成し、送出後は元のタネにも記録が残る）

### 育成度診断（NEW）
- `computePlanSeedReadiness(s)`：コンセプト／視点転換／ログライン／キャラクター／あらすじ／企画書の**6軸**でタネの充実度を判定するヒューリスティック関数。各軸の充足フラグ・充足数・スコア（0-100%）・未充足項目リスト・試用済みレンズ集合を返す
- タネカード一覧（ストリーム表示）に「育成度 ◯%」バッジを表示し、一覧を見るだけでどのタネが企画として熟しているか把握できるようにした（`renderPlanLabSeedCard`）
- 「育てる」タブでは6軸バッジと「次に育てるとよさそうなこと」提案（未充足項目→該当タブへの`switchHubTab`リンク）を表示し、全軸充足時は祝福表示（`success-box`）を出す

### 対応状況
- 【初版実装】JS構文チェック（`node -c`）・ビルド（`vite build`）・minify（`scripts/minify-static.mjs`）・PM2再起動・稼働確認（200 OK）・配信内容確認（`curl`で`企画ラボ`/`planlab-*`クラスの配信を確認）・PlaywrightConsoleCaptureでのページロード確認（コンソールエラー無し）まで完了済み
- 【強化版（8タブ拡張）】ユーザーから「視点を変える・ログライン工房・企画書が原始的で弱い、作品ダッシュボードに負けないレベルに」との指摘を受け、コンセプト／キャラクター／あらすじの3タブ新設と既存3タブの大幅強化、育成度診断機能を追加。JS構文チェック（`node --check`）・ビルド（`vite build`）・minify・PM2再起動・稼働確認（200 OK）・配信内容確認（`curl`で新規関数と新規CSSクラスの配信を確認）まで完了済み
- 実ブラウザでの実際のクリック操作によるE2Eテスト（クイック投入、コンセプト編集、視点カードのクリック、ログライン採用、キャラクター追加、あらすじ執筆、企画書生成、送出ボタン等）は、下記「未完了・引き継ぎ」に記載の理由により本セッションでは未実施。既存の確立パターン（`hubAutoSave`、`renderHubTabs`、`renderHubRichEditor`、`openModal`等）との構造的一致、および`generateLoglines`/`generatePitch`/`addCharacter`との実装同等性をコードレビューで確認済み
- 【専用ページ化（NEW）】モーダル(`openPlanSeedHub`)を廃止し、URLナビゲーション可能な独立ページ`planlab-seed`（`renderPlanLabSeedDash`/`bindPlanLabSeedDash`）に刷新。`navigate()`/popstateハンドラを拡張して`State.currentSeedId`を履歴に保存・復元し、ブラウザバックでもタネ詳細ページへ正しく戻れるようにした。`TOPBAR_PAGES`・`renderLayout`の`isPlanLabPage`判定にも新ページを追加。新規CSSクラス`.pls-*`（ヒーロー・育成度サマリー・ステージロードマップ）を`app.css`に追加。モーダルからページ遷移する箇所（キャラクター編集キャンセル、タネ削除キャンセル）は`closeModal()`を正しく呼ぶよう修正済み。JS構文チェック・ビルド・minify・PM2再起動・稼働確認（200 OK）に加え、一時的なデバッグフック（`?_debugSeed=1`、検証後に削除済み）を用いてPlaywrightConsoleCaptureで実際にタネダッシュボードページと全8タブ（育てる/コンセプト/視点/ログライン/キャラクター/あらすじ/企画書/送る）を描画させ、コンソールエラー0件を確認済み

### ⚠️ 未完了・引き継ぎ
- **実ブラウザでのクリック操作E2Eテスト**：前セッション・本セッションともにPlaywright用Chromiumのインストールを複数回再試行した結果、ダウンロード（177MiB）がネットワーク制約により極端に遅く完了せず、繰り返し試行すると最終的にサンドボックス環境全体がフリーズする（`echo`等の基本コマンドすら応答不能になる）という障害が発生し、`ResetSandbox`での復旧が必要になった。またシステムに既にインストール済みのChromiumバイナリ（`/usr/lib/chromium/chromium-headless-shell`）を`puppeteer-core`経由で直接起動する代替策も試したが、同様にサンドボックス全体がフリーズする障害が再現した。**解決策（本セッションで確立）**：Chromiumプロセスを直接操作せず、既存の安全な`PlaywrightConsoleCapture`ツール（外部で独立したブラウザプロセスを管理）に対し、アプリのHTMLエントリポイントへURLクエリ限定の一時的な自己診断スクリプトを注入する手法を確立。これによりメモ機能について32件のアサーションを実ブラウザDOM上で実行し全PASSを確認できた（詳細は下記「メモ機能」項目を参照）。他の未検証ページについても今後同じ手法を再利用できる
- **世界観設計④（相互リンク機能）**：実装完了。用語集・勢力・場所・年表の相互リンクを追加（詳細は上記セクション参照）
- **企画ラボ**：初版実装＋8タブ拡張強化＋**専用ページ化（本セッション完了）**まで完了（詳細は上記セクション参照）。ユーザーが添付したとされる参考画像2枚は、複数セッションにわたりfilesystem全体を探索したが発見できず、URLも取得できなかった。画像内に追加のデザイン指示が含まれていた可能性があるが、現時点では反映できていない。次回セッションでユーザーに再アップロードを依頼するか、画像なしで進める旨の確認を取ることを推奨する
- **メモ機能（実装完了）**：執筆サポート枠に「グループ＞フォルダ＞メモ」の3階層構造＋タグ/フィルタを備えた高機能メモページ`memo`を新設。データ層は新規`MemoGroupDB`/`MemoFolderDB`/`MemoDB`（`memo_groups`/`memo_folders`/`memo_items`ストア）。`renderMemoPage`（グループサイドナビ＋フォルダバー＋クイック入力＋検索/タグ/タネ紐づきフィルタ＋一括操作＋カードグリッド）、`openMemoHub`（編集タブ＋「タネと紐づけ」タブの2タブ詳細モーダル、`renderHubRichEditor`/`renderHubTagEditor`/`hubAutoSave`を踏襲）、グループ/フォルダ管理モーダルを実装。サイドバー「執筆サポート」・モバイルメニューハブに導線を追加し、モバイルボトムナビ「メモ」ボタンの遷移先を旧`goToScratchpad()`（インスピレーション行き）から新設`goToMemoPage()`に差し替え済み（旧関数は削除）。**さらに企画ラボのタネ専用ダッシュボード（`renderPlanLabSeedDash`）にも9番目のタブ「メモ」を追加**し、`renderPlanLabMemoPanel(s)`／`planlabAddSeedMemo(seedId,pinned)`により、そのタネ・着想に関連した雑案を`MemoDB`の`seedId`フィールドで紐づけて追加・一覧できるようにした（タブバッジには紐づくメモ件数を表示、育成度診断の6軸には含めない設計）。同じメモはグローバルの「メモ」ページ側からも`openMemoHub`内の「タネと紐づけ」タブで確認・解除でき、双方向に集約・管理できる。CSS新規クラス`.memo-page-layout`/`.memo-group-nav`/`.memo-group-item`/`.memo-card-grid`/`.memo-card`/`.memo-seed-chip`等を`app.css`に追加。JS構文チェック（`node --check`）・ビルド（`vite build`）・minify・PM2再起動・稼働確認（200 OK）・PlaywrightConsoleCaptureでのページロード確認（コンソールエラー0件）まで完了。**実ブラウザでの実際のクリック操作E2Eテストも完了（NEW）**：標準のPlaywright Chromiumインストールがネットワーク制約で使えない問題を、`PlaywrightConsoleCapture`（既存の安全な外部ブラウザプロセス管理ツール）に対して`src/index.tsx`へ`?_e2etest=memo`クエリ限定で一時的な自己診断スクリプトを注入する手法で回避し、実ブラウザのDOM上で実際の関数呼び出し（`MemoDB.add`/`openMemoHub`/`planlabAddSeedMemo`等）を実行させて検証。メモ作成・編集・削除、グループ作成・フォルダ作成、タグ付け・タグフィルタ、ピン留めトグル、タネへの紐づけ（`MemoDB.getBySeed`経由の双方向取得）、企画ラボの「メモ」タブでの表示とクイック追加、複数選択・一括削除、フォルダ削除カスケード（配下メモの未分類化）、グループ削除カスケード（配下フォルダも削除・メモは未分類化）、旧`goToScratchpad`の削除確認・新`goToMemoPage`の存在確認まで、**合計32件のアサーションを実行し全てPASS**（コンソールエラー0件）。検証後、一時テストスクリプトは`src/index.tsx`から完全に削除し、`git status`がクリーン（変更ゼロ）であることを確認済み。これによりメモ機能は企画ラボ連携を含めて完成・検証済みの状態
- **GitHubへのpush**：本セッションで解決済み。`setup_github_environment`を再実行し正しい認証情報を取得、リポジトリ`AugWilliamsburg/ScenarioLab`へ`git push -u origin main`成功

### モバイルナビゲーション再編（NEW）
- **課題**：モバイル版ボトムナビが「ホーム/書斎/タスク/メモ/メニュー」の5項目のうち、「メニュー」1つに合計12ページが埋もれて発見しづらい状態だった
- **対応**：①「創作ハブ」新設（`renderCreativeHubPage`、ルート`creative-hub`）で書斎/企画ラボ/ストーリーボード/学習センター/ツール/テンプレートの6ページを集約 ②ボトムナビ「書斎」を「創作」に変更（`isCreativeNavActive`でactive判定） ③「メニュー」ハブを「概要・記録」「設計・資料」「アプリ設定」の3グループ・5ページに再編 ④「メモ」ページに「インスピレーション・ライブラリ」「企画ラボ」への関連リンクを追加
- **結果**：ボトムナビは「ホーム/創作/タスク/メモ/メニュー」の5構成を維持したまま12ページ全てが整理された
- **検証**：構文チェック・ビルド・PM2再起動・稼働確認(200 OK)に加え、`?_e2etest=nav`クエリ限定の一時自己診断スクリプトで`PlaywrightConsoleCapture`実ブラウザ検証、合計40件のアサーション全PASS（コンソールエラー0件）。検証後スクリプトは完全削除し差分ゼロを確認済み

### 「創作」「学ぶ」独自ホーム化 ＋ カテゴリ切替スイッチバー（NEW）
- **課題**：「創作ハブ」「メニューハブ」が単なるカード一覧（メニュー）に留まっており、①ダッシュボード・執筆日誌が創作ハブに未統合 ②「メニュー」という名称が学習系ページの実態と乖離 ③各ページへ切り替えるにはいちいちハブに戻る必要があった
- **対応**：
  1. **「創作ホーム」新設**（`renderCreativeHubPage`刷新、ルート`creative-hub`）：ダッシュボード・執筆日誌を含む7ページ（ダッシュボード/執筆日誌/書斎/企画ラボ/ボード/ツール/テンプレート）を集約。進行中の作品数・今日の執筆字数・育成中タネ数のサマリー統計、進行中作品があれば「続きを書く」導線カード、最近の作品リストを表示する“ホーム”として再設計
  2. **「学ぶホーム」新設**（`renderLearnHubPage`、旧`renderMenuHubPage`を置換、ルート`learn-hub`）：学習センター・名前辞典・世界観設計・設定を集約。既読記事数・登録名前数・学習進捗%のサマリー統計、進行中作品があれば「世界観設計を続ける」導線カードを表示
  3. **カテゴリ切替スイッチバー新設**（`renderCategorySwitchBarHtml` / `.cat-switch-bar` / `.cat-switch-pill`）：画面上部にセグメントコントロール風のピル列を常時表示し、タップ一つで同カテゴリ内の他ページへ即座に切り替え可能に。ホームに戻らなくても「創作」「学ぶ」内の任意ページへワンタップ遷移できる導線を実現（`phase-topbar`と同じ横スクロール可能なピル列パターンを踏襲）
  4. モバイルボトムナビの「メニュー」ボタンを「学ぶ」に変更（アイコン・遷移先とも`learn-hub`に統一）
  5. `renderLayout()`内のカテゴリ判定フラグを再編（`isCreativeNavActive`/`isLearnNavActive`/`activeCatKey`）し、現在ページが属するカテゴリに応じてスイッチバーの表示・active状態を自動制御
- **結果**：ボトムナビ構成は「ホーム/創作/タスク/メモ/学ぶ」の5項目を維持しつつ、「創作」「学ぶ」それぞれが単なるメニューではなく独自のサマリー画面＋ワンタップ切替導線を持つ“ホーム”に進化
- **検証**：構文チェック・ビルド・PM2再起動・稼働確認(200 OK)に加え、`?_e2etest=nav2`クエリ限定の一時自己診断スクリプトで`PlaywrightConsoleCapture`実ブラウザ検証、合計33件のアサーション全PASS（ホーム表示・統計・カード存在・スイッチバー存在とピル数・ピルクリックによる実際のページ遷移・遷移後もスイッチバーが表示されactive状態が追従すること・ボトムナビラベル・トップバータイトルを検証）。検証後スクリプトは完全削除し`git diff src/index.tsx`で差分ゼロを確認済み
