...
- **Local Dev**:
  ...
  ```
- **Last Updated**: 2026-08-17（モバイル版 大幅構造改革 第3弾：スタックナビゲーション風スライド遷移＋戻るボタン中心の階層型ヘッダーを反映）

## モバイル版 大幅構造改革 第3弾：スタックナビゲーション化（NEW・進行中）
...
### 今後の予定（次回セッション以降）
- ホーム(top)ページのモバイル専用レイアウト再設計（現状はPC用2カラムグリッドの縦積みのみで、優先度順の並べ替えや横スクロールカルーセル化などのモバイル専用設計がされていない）
