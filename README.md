...
- **Main Features**:
  ...
  - 執筆サポート：学習センター／ツール／テンプレート／書斎／**企画ラボ（NEW）**
...
- **メモ機能（新規要望・未着手）**：執筆サポート枠に「グループ＞フォルダ＞メモ」の3階層構造＋タグ/フィルタを備えた高機能メモページを新設し、モバイルボトムナビの「メモ」ボタンの遷移先をこの新ページに差し替える要望が出ているが、本セッションではクレジット・時間の制約により全く着手できなかった。次回セッションで最優先事項として着手することを推奨する。設計時は既存の`FolderDB`（単一階層フォルダ）パターンを参考にしつつ、新規に`MemoDB`（グループ／フォルダ／メモの3階層データモデル）を設計する必要がある
