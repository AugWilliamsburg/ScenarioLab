...
- **タネ詳細ハブ**（`openPlanSeedHub`、`renderHubTabs`による5タブ構成）：
  1. **育てる**：タイトル・ステージ・タグの編集、自由メモの追記、成長ログのタイムライン表示
  2. **視点を変える**：AIを使わない固定発想フレーム6種（主客転換／時間を広げる／制約を外す／究極まで縮める／対極をぶつける／当事者の声）をワンクリック適用し、着想を多角的に発展させる（`planlabApplyLens`。適用するとステージが「種」なら自動的に「発芽」へ昇格）
  3. **ログライン工房**：主人公・ゴール・障害を入力すると4パターンのログライン候補をテンプレート補間で生成（`planlabGenerateLoglines`）、採用すると「つぼみ」へ昇格（`planlabChooseLogline`）
  4. **企画書**：ジャンル・テーマ・ターゲットを入力すると、タイトル・ログライン・成長ログの要約（直近5件）を含む企画書ドキュメントを自動生成（`planlabGenerateBrief`）、クリップボードコピー対応（`planlabCopyBrief`）。生成すると「企画書」ステージへ昇格
  5. **送る**：作品タイトルを指定して「作品ダッシュボード」へ新規プロジェクトとして送出するブリッジ機能（`planlabSendToProject`。`newProject`でタイトル・ジャンル・ログライン・あらすじを引き継いで新規作成し、送出後は元のタネにも記録が残る）

### 対応状況
- JS構文チェック（`node -c`）・ビルド（`vite build`）・minify（`scripts/minify-static.mjs`）・PM2再起動・稼働確認（200 OK）・配信内容確認（`curl`で`企画ラボ`/`planlab-*`クラスの配信を確認）・PlaywrightConsoleCaptureでのページロード確認（コンソールエラー無し）まで完了済み
- 実ブラウザでの実際のクリック操作によるE2Eテスト（クイック投入、視点カードのクリック、ログライン採用、企画書生成、送出ボタン等）は、下記「未完了・引き継ぎ」に記載の理由により本セッションでは未実施。既存の確立パターン（`hubAutoSave`、`renderHubTabs`、`openModal`等）との構造的一致をコードレビューで確認済み
...
- **企画ラボ**：実装完了（詳細は上記セクション参照）。ユーザーが添付したとされる参考画像2枚は、複数セッションにわたりfilesystem全体を探索したが発見できず、URLも取得できなかった。画像内に追加のデザイン指示が含まれていた可能性があるが、現時点では反映できていない。次回セッションでユーザーに再アップロードを依頼するか、画像なしで進める旨の確認を取ることを推奨する
